"""Make an ADK agent's A2UI output render in the ADK dev UI (`adk web`) or frontend proxy.

`adk web` and custom A2UI renderers expect A2UI JSON payload structured cleanly.
This callback converts A2UI JSON emitted by models into formatted data parts.
"""
...
def _wrap_a2ui_part(a2ui_message: dict) -> types.Part:
    """Wrap a single A2UI message for rendering."""
...
def _sanitize_image_components(messages: list[dict]) -> None:
    """Replace un-fetchable <Image> components with a short <Text> note."""
    for m in messages:
...
def _surface_is_renderable(messages: list[dict]) -> bool:
    """True only if the messages form a surface adk web / chat UI can actually draw."""
    all_ids: set = set()
    all_refs: set = set()
    roots: list = []
    has_body = False
